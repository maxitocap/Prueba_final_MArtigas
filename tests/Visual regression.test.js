...
describe('Test de regresión visual del header', () => {
  it('Comparación de imágenes con página de Madison Island', async () => {
    ...
    expect(
      await browser.checkElement('header', 'page-header', {}), "Error: el header de Madison Island no coincide con la original").equal(0);
    ...
    await $('#header').waitForDisplayed();
    expect(await browser.checkElement('header', 'page-header', {})).to.not.equal(0);
  })})
